...
class upPatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=16, in_ch=3, out_ch=768, with_pos=False):
        super().__init__()
        self.patch_size = to_2tuple(patch_size)
        self.conv = nn.ConvTranspose2d(in_ch, out_ch, kernel_size=patch_size+2, stride=patch_size, padding=patch_size // 2)
        self.norm = nn.BatchNorm2d(out_ch)
...
class upBasicStem(nn.Module):
    def __init__(self, in_ch=64, out_ch=3, with_pos=False):
        super(upBasicStem, self).__init__()
        hidden_ch = in_ch // 2  #分母最初为2
        self.conv1 = nn.ConvTranspose2d(in_ch, hidden_ch, kernel_size=4, stride=2, padding=1, bias=False)
        #self.norm1 = nn.BatchNorm2d(hidden_ch)
        #self.conv2 = nn.ConvTranspose2d(hidden_ch, hidden_ch // 2, kernel_size=4, stride=2, padding=1, bias=False)
        self.conv2 = nn.Conv2d(hidden_ch, hidden_ch, kernel_size=3, stride=1, padding=1, bias=False)
        #self.norm2 = nn.BatchNorm2d(hidden_ch // 2)
        self.conv3 = nn.ConvTranspose2d(hidden_ch, out_ch, kernel_size=4, stride=2, padding=1, bias=False)
        #self.conv3 = nn.Conv2d(hidden_ch // 2, out_ch, kernel_size=3, stride=1, padding=1, bias=False)
        self.act = nn.ReLU(inplace=True)
        self.with_pos = with_pos
        if self.with_pos:
            self.pos = PA(out_ch)
...
class ResT(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 qk_scale=None, drop_rate=0.5, attn_drop_rate=0.5, drop_path_rate=1/2,
                 depths=[2, 2, 2, 2, 2, 2, 2, 2], sr_ratios=[8, 4, 2, 1],
        ...
        self.patch_embed_5 = PatchEmbed(patch_size=2, in_ch=embed_dims[3], out_ch=embed_dims[3]*2, with_pos=True)

        self.uppatch_embed_5 = upPatchEmbed(patch_size=2, in_ch=embed_dims[3]*2, out_ch=embed_dims[3], with_pos=True)

        self.uppatch_embed_4 = upPatchEmbed(patch_size=2, in_ch=embed_dims[3] * 2, out_ch=embed_dims[2], with_pos=True) #5层编码
        #self.uppatch_embed_4 = upPatchEmbed(patch_size=2, in_ch=embed_dims[3], out_ch=embed_dims[2], with_pos=True) #4层编码

        self.uppatch_embed_3 = upPatchEmbed(patch_size=2, in_ch=embed_dims[2]*2, out_ch=embed_dims[1], with_pos=True)
        self.uppatch_embed_2 = upPatchEmbed(patch_size=2, in_ch=embed_dims[1]*2, out_ch=embed_dims[0], with_pos=True)
        ...
        self.mid = Block(embed_dims[3] * 2, num_heads[3], mlp_ratios[3], qkv_bias, qk_scale, drop_rate, attn_drop_rate,
                  drop_path=dpr[cur], norm_layer=norm_layer, sr_ratio=sr_ratios[3], apply_transform=apply_transform)

        # transformer decoder
        cur += depths[3]
        self.stage5 = nn.ModuleList([
            Block(embed_dims[3]*2, num_heads[3], mlp_ratios[3], qkv_bias, qk_scale, drop_rate, attn_drop_rate,
                  drop_path=dpr[cur + i], norm_layer=norm_layer, sr_ratio=sr_ratios[3], apply_transform=apply_transform)
            for i in range(self.depths[4])])

        cur += depths[4]
        self.stage6 = nn.ModuleList([
            Block(embed_dims[2] * 2, num_heads[2], mlp_ratios[2], qkv_bias, qk_scale, drop_rate, attn_drop_rate,
                  drop_path=dpr[cur + i], norm_layer=norm_layer, sr_ratio=sr_ratios[2], apply_transform=apply_transform)
            for i in range(self.depths[5])])

        cur += depths[5]
        self.stage7 = nn.ModuleList([
            Block(embed_dims[1]*2, num_heads[1], mlp_ratios[1], qkv_bias, qk_scale, drop_rate, attn_drop_rate,
                  drop_path=dpr[cur + i], norm_layer=norm_layer, sr_ratio=sr_ratios[1], apply_transform=apply_transform)
        ...
        self.stage8 = nn.ModuleList([
            Block(embed_dims[0]*2, num_heads[0], mlp_ratios[0], qkv_bias, qk_scale, drop_rate, attn_drop_rate,
                  drop_path=dpr[cur + i], norm_layer=norm_layer, sr_ratio=sr_ratios[0], apply_transform=apply_transform)
            for i in range(self.depths[7])])

        #self.upstem = upBasicStem(in_ch=embed_dims[0]*2, out_ch=in_chans, with_pos=True)
        self.upstem = upBasicStem(in_ch=embed_dims[0] * 2, out_ch=32, with_pos=True)
        #self.upstem = upBasicStem(in_ch=embed_dims[0] * 3, out_ch=32, with_pos=True)


        # self.norm = norm_layer(embed_dims[0]*2)
        # self.output = nn.Sequential(nn.Conv2d(in_channels=12, out_channels=12, kernel_size=3, padding=1))
    ...
    def forward(self, x):
        ...
        x4 = x
        x = x.permute(0, 2, 1).reshape(B, -1, H, W)
        #print('p4 output',x.shape, H, W)

        x, (H, W) = self.patch_embed_5(x)
        x = self.mid(x, H, W)
        x = x.permute(0, 2, 1).reshape(B, -1, H, W)
        #print(x.shape)

        x, (H, W) = self.uppatch_embed_5(x)
        decoder_1 = torch.cat([x, x4], -1)
        #print(decoder_1.shape)
        for blk in self.stage5:
            decoder_1 = blk(decoder_1, H, W)
        decoder_1 = decoder_1.permute(0, 2, 1).reshape(B, -1, H, W)
        #print('decoder1 shape',decoder_1.shape)

        x, (H, W) = self.uppatch_embed_4(decoder_1)
        decoder_2 = torch.cat([x, x3],-1)
        for blk in self.stage6:
            decoder_2 = blk(decoder_2, H, W)
        decoder_2 = decoder_2.permute(0, 2, 1).reshape(B, -1, H, W)
        #print(decoder_2.shape)

        x, (H, W) = self.uppatch_embed_3(decoder_2)
        decoder_3 = torch.cat([x, x2], -1)
        for blk in self.stage7:
            decoder_3 = blk(decoder_3, H, W)
        decoder_3 = decoder_3.permute(0, 2, 1).reshape(B, -1, H, W)
        #print(decoder_3.shape)

        x, (H, W) = self.uppatch_embed_2(decoder_3)
        decoder_4 = torch.cat([x,x1],-1)
        for blk in self.stage8:
            decoder_4 = blk(decoder_4, H, W)
        #decoder_4 = self.norm(decoder_4)
        decoder_4 = decoder_4.permute(0, 2, 1).reshape(B, -1, H, W)
        #print(decoder_4.shape)

        # x,(H,W) = self.uppatch_embed_1(decoder_4)
        # x = x.permute(0, 2, 1).reshape(B, -1, H, W)
        # print(x.shape)
        #x = torch.cat([x0,decoder_4],1)
        x = self.upstem(decoder_4)
        #x = self.upstem(x)
        #x = self.up(decoder_4)
        #x = self.output(x)
        return x
...
if __name__ == '__main__':
    Rest = ResT(embed_dims=[96, 192, 384, 768], num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
                 depths=[2, 2, 18, 2, 2, 18, 2, 2], sr_ratios=[8, 4, 2, 1], apply_transform=True)
    img = torch.randn(2,3,480,512)
    y = Rest(img)
    print(y.shape)